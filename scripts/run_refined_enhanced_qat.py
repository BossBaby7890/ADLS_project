...
logger = logging.getLogger("run_refined_enhanced_qat")
...
def run_adaround_with_schedule(
    ...
    for layer_name, bit_cfg in bit_map.items():
        ...
        logger.info(
            "Refined Adaptive AdaRound [%d/%d] %s | bits=%d | sens=%.4f | steps=%d | calib_batches=%d",
    ...
    logger.info("Refined Adaptive AdaRound complete.")


def parse_args():
    parser = argparse.ArgumentParser(description="Refined enhanced APQ-Lite pipeline")
...
def main():
    ...
    generator.save(chop_config, out_dir / "quant_config_refined_enhanced.json")
    ...
    with open(out_dir / "bit_refinement_report.json", "w") as fh:
        json.dump(refinement_report, fh, indent=2)
    logger.info("Bit refinement report saved to %s", out_dir / "bit_refinement_report.json")

    ckpt_dir = Path(cfg["project"]["checkpoint_dir"])
    ckpt_dir.mkdir(parents=True, exist_ok=True)

    adaround_ckpt_path = ckpt_dir / "checkpoint_refined_adarounded_enhanced.pth"
    torch.save({"model_state_dict": model.state_dict()}, adaround_ckpt_path)
    logger.info("Refined AdaRounded checkpoint saved to %s", adaround_ckpt_path)
    ...
    try:
        ...
        quant_ckpt_path = ckpt_dir / "checkpoint_refined_quantized_enhanced.pth"
        torch.save({"model_state_dict": model.state_dict()}, quant_ckpt_path)
        logger.info("Refined quantized checkpoint saved to %s", quant_ckpt_path)

    except ModuleNotFoundError:
        logger.warning(
            "CHOP/MASE is not installed in this environment. "
            "Saved refined AdaRounded checkpoint and quant config only. "
